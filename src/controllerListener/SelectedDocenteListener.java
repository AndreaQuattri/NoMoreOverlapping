...
public class SelectedDocenteListener implements  ActionListener
	...
	public SelectedDocenteListener(Model model, ViewTimeTable viewOrario) {
		...
		public void actionPerformed(ActionEvent action)
		{
			...
			for (int i=0; i<model.getListOrario().size(); i++){
				...
				for (int j=0; j<model.getListOrario().get(i).getElencoAssegnamenti().size(); j++){
					...
					app = viewOrario.getComboBoxDocenti().getSelectedItem().toString().split("-");

					for (int iDoc = 0; iDoc < model.getListOrario().get(i).getElencoAssegnamenti().get(j).getAttività().getElencoResponsabili().size(); iDoc++){
						if (model.getListOrario().get(i).getElencoAssegnamenti().get(j).getAttività().getElencoResponsabili().get(iDoc).getMatricola().equals(app[0].trim()))
							countDay++;
					}
				}
				if (iColonne == 0){
